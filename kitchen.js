  ...
  profilesListEl.querySelectorAll(".btnDelProfile").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      try{
        ...
        uiMessage("Perfil guardado correctamente.");
      }finally{
        hideLoading();
      }
    });
...
btnAddProfile.addEventListener("click", async ()=>{
  ...
  try{
    ...
    uiMessage("No se pudo agregar.", true);
  }finally{
    hideLoading();
  }
});
...
// === Mejora UX: reemplazar confirm() y alert() ===

function uiMessage(msg, isError=false){
  ...
  box.style.background = isError ? "#b00020" : "#2e7d32";
